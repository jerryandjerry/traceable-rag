...
def tokenize_chunks(chunks, doc, eng, pdf_parser=None, chunk_images=None):
    res = []
    # wrap up as es documents
    for i, ck in enumerate(chunks):
        ...
        if pdf_parser:
            try:
                # pdf_parser.crop() crops an image from the PDF based on the chunk's position information
                d["image"], poss = pdf_parser.crop(ck, need_position=True)
                add_positions(d, poss)
                ck = pdf_parser.remove_tag(ck)
                
                # Extract tbls images from the chunk
                d["ref_images"] = []
                if chunk_images and i < len(chunk_images) and chunk_images[i]:
                    d["ref_images"] = chunk_images[i]
