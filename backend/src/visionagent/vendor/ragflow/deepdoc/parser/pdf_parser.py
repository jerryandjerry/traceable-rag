...
from visionagent.vendor.ragflow.deepdoc.vision import (
    OCR,
    LayoutRecognizer,
    Recognizer,
...
def _looks_english(texts, *, minimum_chars=30, latin_ratio=0.8):
    """Classify extracted text without changing parser behaviour at random.

    Language detection decides whether PDF text is retained or discarded
    before OCR.  The previous unseeded random sample allowed identical input
    to enter different OCR paths.  Inspect the available letters directly;
    pages containing only numbers and punctuation use an ASCII fallback.
    """
    ...
    if letters:
        ...
        return latin / len(letters) > latin_ratio

    ascii_chars = sum(char.isascii() for char in visible)
    return ascii_chars / len(visible) > latin_ratio
...
class RAGFlowPdfParser:
    ...
    def __ocr(self, pagenum, img, chars, ZM=3):
        ...
        # Convert zip to list to get length, but be careful with memory
        bxs_list = list(bxs) if bxs else []
        bxs = bxs_list  # Use the list version
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        try:
            ...
            try:
                ...
                total_chars = sum(len(chars) for chars in self.page_chars)
                self.total_chars_extracted = total_chars
            except Exception:
                logger.warning(
                    "failed to extract PDF characters for page range",
                    extra={"page_from": page_from, "page_to": page_to},
                    exc_info=True,
                )
            ...
            raise Exception(f"Failed to load PDF file: {fnm}. Error: {e}")
        ...
        try:
            ...
            logger.warning("PDF outline extraction failed", exc_info=True)
        if not self.outlines:
            logger.debug("PDF has no outlines")

        logger.debug("PDF pages converted to images")
        english_pages = [
            _looks_english(c["text"] for c in page_chars)
            for page_chars in self.page_chars
        ]
        self.is_english = sum(english_pages) > len(self.page_images) / 2
        # st = timer()
        for i, img in enumerate(self.page_images):
            ...
            # Report progress more frequently (every page instead of every 6 pages)
            if callback:
                progress = (i + 1) * 0.6 / len(self.page_images)
                message = f"OCR processing page {i + 1}/{len(self.page_images)}"
        ...
        if not self.is_english and not any(
            ...
            self.is_english = _looks_english(b["text"] for b in bxes)
        logger.debug("PDF language detection completed", extra={"is_english": bool(self.is_english)})

        self.page_cum_height = np.cumsum(self.page_cum_height)
        assert len(self.page_cum_height) == len(self.page_images) + 1
        if len(self.boxes) == 0 and zoomin < 9:
            logger.debug("PDF OCR retrying at higher zoom", extra={"zoomin": zoomin * 3})
